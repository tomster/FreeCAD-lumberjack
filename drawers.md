...
- **mitered** (formerly "overlap"): corners cut by hand. we don't compute the joints themselves but simply dimension the panels so that they all fully overlap (front and back run the full `width`); no corner pocket is cut.
- **finger joint**: a box joint with square fingers, modelled in the bodies. all four walls run full size like the mitered variant. an **even** number `n = 2 * max(1, round(height / (2 * t_side)))` of fingers of pitch `p = height / n` at every corner (exactly `t_side` wide when the height is an even multiple of it; 12.5 mm for 100 mm / 12 mm, 10.5 mm for 84 mm / 12 mm), `t_side` deep, through the thickness. the **sides** carry the teeth at the even positions counted from the bottom edge (a tooth at the bottom edge), the **front and back** at the odd ones. with an even count the two patterns are each other's upside-down image, which is what allows a single vertical CAM job for all four walls. every slot is cut `finger_tolerance` (default 0.05 mm) wider on each flank, which makes the teeth thinner by the same amount, so the mating clearance is `4 * finger_tolerance` per finger. the slots are one pocket per wall (a rectangle at each end) repeated along the height by a linear pattern whose occurrence count is an expression, so height changes stay live. the fingers are **not** cut on the sheet (the walls are nested as plain rectangles); see the CAM section for the vertical finger jobs.

all pockets run the full height of the panel. the sides' end grain shows at the drawer face in every variant unless a dedicated drawer front covers it (with finger joints the front's end grain shows on the sides as well).

## finger joint CAM

the fingers cannot be cut with the panel lying flat, so a run of *Drawer CAM Job* adds one extra job per group of finger-jointed drawers with the same `height`, `t_side` and `finger_tolerance`, holding **all four walls** of every drawer of the group in one pack. the panels stand on end on the bed, stacked face to face along +Y from Y = 0 (in any order), the end face to be cut flush at Z = 0 (lower-left origin, no orientation options): the **sides** with their bottom (grooved) edge at X = 0, the **fronts and backs upside down** (grooved edge at X = `height`) — because the finger count is even, a flipped front shows exactly the sides' slot pattern. the stock is the pack's envelope. every slot is a set of parallel slot passes across the whole pack, `t_side` deep, running 5 mm plus a tool radius past both outer faces into sacrificial boards clamped there against tear-out (keep them flush with the end faces). the job is run **once per end**: turn the pack end over end, keeping each panel's edges where they were. the bit must be thinner than `t_side`; the run summary states the pack thickness, slot and pass counts and the clamping convention. G-code: `<Document>_<container>_fingers_<height>mm.nc`. no overview page is generated for finger jobs yet.
...
beneath those is a combo box `corner_joint` selecting the corner joinery (tongue and dado recessed / half-lap / mitered / tongue and dado flush / finger joint, see above); with mitered and finger joints the dimensions of the box parts overlap, otherwise they are dimensioned to consider the joinery. a `finger_tolerance` field (default 0.05 mm) is enabled for the finger joint only.
