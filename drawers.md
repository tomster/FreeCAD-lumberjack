...
the fingers cannot be cut with the panel lying flat, so a run of *Drawer CAM Job* adds two extra jobs per group of finger-jointed drawers with the same `height`, `t_side` and `finger_tolerance`: a **sides** job (SideL and SideR of every drawer of the group, the "male" pattern) and a **fronts** job (Front and Back, "female"). the panels stand on end on the bed, stacked face to face along +Y from Y = 0, their bottom (grooved) edges at X = 0, the end face to be cut flush at Z = 0 (lower-left origin, no orientation options). the stock is the stack's envelope. every slot is a set of parallel slot passes across the whole stack, `t_side` deep, running 5 mm plus a tool radius past both outer faces into sacrificial boards clamped there against tear-out (keep them flush with the end faces). the job is run **once per end**: turn the stack end over end, keeping the bottom edges at X = 0. the bit must be thinner than `t_side`; the run summary states the stack thickness, slot and pass counts and the clamping convention. G-code: `<Document>_<container>_fingers_<height>mm_<sides|fronts>.nc`. no overview page is generated for finger jobs yet.
